...
GLuint renderer_t::makeGear(const GLfloat *reflectance, GLdouble innerRadius,
...
void renderer_t::drawGear(GLuint gear, GLdouble dx, GLdouble dy, GLdouble dz,
                        GLdouble angle)
{
   glPushMatrix();
...
void renderer_t::mousePressEvent(QMouseEvent *event)
{
   lastPos = event->pos();
}

void renderer_t::setXRotation(int angle)
{
...
void renderer_t::setYRotation(int angle)
{
   normalizeAngle(&angle);
   if (angle != yRot) {
      yRot = angle;
...
void renderer_t::setZRotation(int angle)
{
   normalizeAngle(&angle);
   if (angle != zRot) {
      zRot = angle;
...
void renderer_t::normalizeAngle(int *angle)
{
   while (*angle < 0)
      *angle += 360 * 16;
   while (*angle > 360 * 16)
