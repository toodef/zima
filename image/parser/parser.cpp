...
tif_parser_t::tif_parser_t( std::string const & file )
{
   ...
   raster_band_ = GDALGetRasterBand(data_, 1);

   x_size_ = (size_t)GDALGetRasterBandXSize(raster_band_);
   y_size_ = (size_t)GDALGetRasterBandYSize(raster_band_);
}
...
float * tif_parser_t::parse()
{
   ...
   CPLErr err = GDALRasterIO(raster_band_, GF_Read, 0, 0, (int) x_size_, (int) y_size_, image, (int) x_size_, (int) y_size_, GDT_Float32, 0, 0);

   int got_min, got_max;
   min_ = (float)GDALGetRasterMinimum(raster_band_, &got_min);
   max_ = (float)GDALGetRasterMaximum(raster_band_, &got_max);

   double min_max[2];

   if (!(got_max && got_min)) {
      GDALComputeRasterMinMax(raster_band_, false, min_max);
